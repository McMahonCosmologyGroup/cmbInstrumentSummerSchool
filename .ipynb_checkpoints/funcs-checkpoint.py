...
def getData1D(dataParams, team):
    ...
    FILE_NAME = (
        ...
        + team
        + "/"
        + str(dataParams.frequency)
        + "GHz_"
        + str(dataParams.sweep_ang)
        + "deg_1D_"
        + dataParams.cut
        + ".txt"
    )
    ...
    A = amp_cross / np.max(amp_cross)
    beam_complex = A * np.exp(amp_phase * np.pi / 180.0 * complex(0, 1))
...
def getData2D(txt_file):
    ...
    while jj <= 1:
        ...
        while i < (nsamp):
            ...
            arr_AA = np.array(
                running_mean(DATA[i][i_AA_begin:i_AA_end], RoachOpt.L_MEAN)
            )
            arr_BB = np.array(
                running_mean(DATA[i][i_BB_begin:i_BB_end], RoachOpt.L_MEAN)
            )
            arr_AB = np.array(
                running_mean(DATA[i][i_AB_begin:i_AB_end], RoachOpt.L_MEAN)
            )
            ...
            phase[i] = np.remainder(arr_phase[int(n_channels / 2)], 360.0)
    ...
    beam_complex = P * np.exp(Z * np.pi / 180.0 * complex(0, 1))
...
def coords_ang_to_spat(theta, freq):
    ...
    x_spat = (
        np.linspace(-int((len(theta) / 2)), int((len(theta) / 2)), int((len(theta))))
        * delta_x
        * 1e2
    )
